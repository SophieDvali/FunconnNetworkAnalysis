...
df_C_values = pd.read_csv('/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/SMBthings/C_values_mcmc.csv',
                          header=None)
...
frac_asso = len(np.where(relationships == 1)[1])/(len(groups_heirarchical_unique)*(len(groups_heirarchical_unique)-1))
frac_cp = len(np.where(relationships == 3)[1])/(len(groups_heirarchical_unique)*(len(groups_heirarchical_unique)-1))
...
#the following are all nonoverlapping
...
for r in np.arange(npermute):
    ...
    permutation = np.random.permutation(np.arange(len(groups_heirarchical)))
...
#diss is NaN so we do not inclue, now we have Assortative, Core and Periphery
C_SIM_pvalues_nonNaN = C_SIM_pvalues[~np.isnan(C_SIM_pvalues)]
#C_SIM_pvalues_flattened = C_SIM_pvalues.flatten()
C_SIM_pvalues_corrected = stats.false_discovery_control(C_SIM_pvalues_nonNaN)
C_SIM_pvalues_corrected_matrix = C_SIM_pvalues_corrected.reshape(3,3)
...
C_function_pvalues_corrected_matrix = C_function_pvalues_corrected.reshape(6,3)
...
C_richclub_sp_pvalues_corrected_matrix = C_richclub_sp_pvalues_corrected.reshape(3,1)
...
C_richclub_aconn_pvalues_corrected_matrix = C_richclub_aconn_pvalues_corrected.reshape(3,1)


other_js = [0,2,3]
fig, ax = plt.subplots()
plt.imshow(C_richclub_sp_zscore.reshape(4,1), cmap="coolwarm", vmin = -2, vmax = 2)
for i in np.arange(3):
        if C_richclub_sp_pvalues_corrected_matrix[i] < 0.01:
            ax.text(0,other_js[i], "**", va='center', ha='center')
...
plt.savefig("/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/SMBthings/Richclub_ACDP_zscores_mcmc.pdf", dpi=300, bbox_inches="tight")
plt.savefig(
    "/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/ForFigures/Fig5/Richclub_ADCP_zscores_mcmc.eps",
    dpi=300, bbox_inches="tight", format="eps")
plt.show()

fig, ax = plt.subplots()
plt.imshow(C_richclub_aconn_zscore.reshape(4,1), cmap="coolwarm", vmin = -2, vmax = 2)
for i in np.arange(3):
        if C_richclub_aconn_pvalues_corrected_matrix[i] < 0.01:
            ax.text(0,other_js[i], "**", va='center', ha='center')
        elif C_richclub_aconn_pvalues_corrected_matrix[i] < 0.05:
            ax.text(0,other_js[i], "*", va='center', ha='center')
ax.set_xticklabels(["", "Anatomical Rich Club"])
plt.yticks(np.arange(4),["Assortative", "Dissassortative",  "Core", "Periphery"])
#ax.set_ylabel("Block")
plt.xticks(rotation=90)
plt.tight_layout()
cbar = plt.colorbar()
cbar.set_label('z-score', rotation=270)
plt.savefig("/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/SMBthings/AconnRichclubinSPmodules_ACDP_zscores_mcmc.pdf", dpi=300, bbox_inches="tight")
plt.savefig(
    "/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/ForFigures/Fig5/AconnRichclubinSPmodules_ADCP_zscores_mcmc.eps",
    dpi=300, bbox_inches="tight", format="eps")
plt.show()
...
fig, ax = plt.subplots()
plt.imshow(C_SIM_zscore[~np.isnan(C_SIM_zscore)].reshape(3,3), cmap="coolwarm")
for i in np.arange(3):
...
ax.set_yticklabels(["", "Assortative", "", "Core", "","Periphery"])
#ax.set_ylabel("Block")
plt.xticks(rotation=90)
plt.tight_layout()
cbar = plt.colorbar()
cbar.set_label('z-score', rotation=270)
plt.savefig("/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/SMBthings/SIM_ACDP_zscores_mcmc.pdf", dpi=300, bbox_inches="tight")
plt.show()

other_js = [0,2,3]
fig, ax = plt.subplots()
plt.imshow(C_function_zscore, cmap="coolwarm", vmin = -3, vmax = 3)
for i in np.arange(6):
    for j in np.arange(3):
        if C_function_pvalues_corrected_matrix[i,j] < 0.01:
            ax.text(other_js[j], i, "**", va='center', ha='center')
...
plt.savefig("/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/SMBthings/neuronfunc_ADCP_zscores_mcmc_other.pdf", dpi=300, bbox_inches="tight")
plt.savefig(
    "/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/ForFigures/Fig4/neuronfunc_ADCP_zscores_mcmc_other.eps",
    dpi=300, bbox_inches="tight", format="eps")
plt.show()
...
other_js = [0,2,3]
fig, ax = plt.subplots()
plt.imshow(C_function_zscore.transpose(), cmap="coolwarm", vmin = -3, vmax = 3)
for i in np.arange(6):
    for j in np.arange(3):
        if C_function_pvalues_corrected_matrix[i,j] < 0.01:
            ax.text(i, other_js[j], "**", va='center', ha='center')
        elif C_function_pvalues_corrected_matrix[i,j] < 0.05:
            ax.text(i, other_js[j], "*", va='center', ha='center')
...
plt.xticks(np.arange(3),["Assortative",  "Core", "Periphery"])
plt.xticks(rotation=90)
plt.tight_layout()
plt.colorbar().ax.set_ylabel('corrected p-values', rotation=270, labelpad=15)
plt.savefig("/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/SMBthings/neuronfunc_ADCP_pvals_mcmc.pdf", dpi=300, bbox_inches="tight")
plt.show()
...
plt.savefig("/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/SMBthings/SIM_ADCP_pvals_mcmc.pdf", dpi=300, bbox_inches="tight")
plt.show()


# 1 is asso, 2 is diss, 3 is core, 4 is periphery
C_class = []
for i in np.arange(len(C_values)):
    if C_values[i][0] == max(C_values[i][0:4]):
        C_class.append(1)
    if C_values[i][1] == max(C_values[i][0:4]):
        C_class.append(2)
    if C_values[i][2] == max(C_values[i][0:4]):
        C_class.append(3)
    if C_values[i][3] == max(C_values[i][0:4]):
        C_class.append(4)

fractions = []
for a in np.arange(4):
    fractions.append(len(np.where(np.array(C_class) == 1 + a)[0]))

#ok maybe this is not the best way to do this since this jest looks at the largest one lets do average fraction of time a
#node participates in each so that would just be a sum divided by 188

C_mean = np.mean(C_values, axis = 0)

plt.figure()
plt.pie(C_mean[0:4], labels=["Assortative", "Disassortative", "Core", "Periphery"])
plt.savefig("/Users/sophiedvali/Documents/University/Research/NetworkAnalysis/SMBthings/ADCP_frac_score_mcmc.pdf",
